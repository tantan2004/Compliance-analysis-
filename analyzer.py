...
class Analysis:
    ...
    def _setup_analysis_chain(self):
        ...
        tool_qa = Tool(
            ...
            description=(
                "Use this tool to answer compliance questions from the uploaded document. "
                "Useful for summarizing or asking about rules, obligations, or risk items."
            )
        )
        ...
        tool_rule = Tool(
            ...
            description=(
                "Scans the document for rule violations using predefined regex rules "
                "and validates them using the LLM. Call this to get a list of flagged compliance issues."
            )
        )
        ...
        tool_risk = Tool(
            ...
            description=(
                "Assesses the overall compliance risk level (Low, Medium, High) "
                "based on detected violations in the document."
            )
        )
    ...
    def _extract_context(self, text, start, end, window=100):
       """
       Extracts a snippet of text surrounding a regex match to provide context.
       """
    ...
    def analyze_file(self, file_path, query=None):
        ...
        rule_violations = self.rule_based_scan(raw_text)
